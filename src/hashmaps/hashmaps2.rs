...
/*
    특정 키가 값과 함께 해시맵에 이미 존재하는지 확인한 후 다음 동작을 실행하는 것이 일반적이다.
    키가 해시맵에 존재하는 경우 기존 값은 그대로 유지되어야 한다. 키가 없으면 키와 값을 삽입한다.

    .entry()는 해시맵에 확인하려는 키를 매개변수로 사용하는 특수한 API이다. 매서드의 반환값은 존재하거나
    존재하지 않을 수 있는 값을 나타내는 열거형(enum)이다.

    .or_insert() 메서드는 .enrty()로 확인한 경우 없다면 메서드 내의 값을 삽입하는 것이다.
*/
